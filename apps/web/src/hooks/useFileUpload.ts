...
export function useFilesUpload() {
  const { setUploads, uploads, confirmUpload } =
    useFiles()
  ...
  const mutation = useMutation({
      ...
      const uploadTasks = files.map(fileData => {
        return limit(async () => {
          ...
          try {
            ...
            successUploads.current.push(
              fileData.fileId,
            )
            if (
              successUploads.current.length >=
              batchSize
            ) {
              const confirmFiles = [
    ...
    onSettled: async () => {
      if (successUploads.current.length > 0) {
        const remainingUploadedFiles = [
          ...successUploads.current,
        ]
        successUploads.current = []
        const result =
          await confirmUpload.mutateAsync(
            remainingUploadedFiles,
          )
        console.info(result)
      }
    },
